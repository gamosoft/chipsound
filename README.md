...
- The samples pane starts **automatic**: as wide as the longest sample name (floor 120px, cap 40% of the row). Drag the handle on its inner edge to set a width for this browser; double-click the handle (or Home with it focused) to go back to automatic. It is tabbed **SAMPLES** / **PLAYLIST** — click a playlist row to jump, hover a row for the trash can to remove it. Trash on the current track loads the next one (stopped); trash on the last track, or Clear playlist, unloads the player. `I` still hides the whole rail.
...
| Playlist trash | Remove that track (next loads, stopped; last one unloads) |
